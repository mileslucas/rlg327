...
#define distxy(x, y) (d->dist_map[y][x])
#define disttunxy(x, y) (d->dist_tun_map[y][x])
...
typedef struct dungeon {
  ...
  uint32_t dist_map[DUNGEON_Y][DUNGEON_X];
  uint32_t dist_tun_map[DUNGEON_Y][DUNGEON_X];
} dungeon_t;
...
static void dijkstra_distance(dungeon_t *d, pair_t from, pair_t to){
	static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      distxy(from[dim_x], from[dim_y]) = p->cost;
      heap_delete(&h);
...
static void dijkstra_tunnel_distance(dungeon_t *d, pair_t from, pair_t to){
	static corridor_path_t path[DUNGEON_Y][DUNGEON_X], *p;
  static uint32_t initialized = 0;
  heap_t h;
  uint32_t x, y;
  uint32_t weight[DUNGEON_Y][DUNGEON_X];
  ...
  while ((p = heap_remove_min(&h))) {
    ...
    if ((p->pos[dim_y] == to[dim_y]) && p->pos[dim_x] == to[dim_x]) {
      disttunxy(from[dim_x], from[dim_y]) = p->cost;
      heap_delete(&h);
	...
	if ((path[p->pos[dim_y] - 1][p->pos[dim_x] - 1   ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x] - 1   ].cost >
         p->cost + weight[p->pos[dim_y] - 1][p->pos[dim_x] - 1])) {
      path[p->pos[dim_y] - 1][p->pos[dim_x] - 1   ].cost =
        p->cost + weight[p->pos[dim_y] - 1][p->pos[dim_x] - 1];
      path[p->pos[dim_y] - 1][p->pos[dim_x]  - 1  ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]  - 1  ].from[dim_x] = p->pos[dim_x];
    ...
    if ((path[p->pos[dim_y] - 1][p->pos[dim_x]    ].hn) &&
        (path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost >
         p->cost + weight[p->pos[dim_y] - 1][p->pos[dim_x]])) {
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].cost =
        p->cost + weight[p->pos[dim_y] - 1][p->pos[dim_x]];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y] - 1][p->pos[dim_x]    ].from[dim_x] = p->pos[dim_x];
    ...
    if ((path[p->pos[dim_y]    ][p->pos[dim_x] - 1].hn) &&
        (path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost >
         p->cost + weight[p->pos[dim_y]][p->pos[dim_x] - 1])) {
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].cost =
        p->cost + weight[p->pos[dim_y]][p->pos[dim_x] - 1];
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_y] = p->pos[dim_y];
      path[p->pos[dim_y]    ][p->pos[dim_x] - 1].from[dim_x] = p->pos[dim_x];
...
int gen_distance(dungeon_t *d){

  int i, j;
  pair_t from, to;
	to[dim_y] = player[dim_y];
	to[dim_x] = player[dim_x];

  for(i = 0; i < DUNGEON_Y; i++) {
    for(j = 0; j < DUNGEON_X; j++) {
    	from[dim_y] = i;
        from[dim_x] = j;
      if (mapxy(j, i) >= ter_floor) {
        dijkstra_distance(d, from, to);
      } else {
      	distxy(j, i) = 255;
      }

      if (mapxy(j, i) != ter_wall_immutable) {
     	 dijkstra_tunnel_distance(d, from, to);
		} else {
			disttunxy(j, i) = 255;
		}
    }
...
int gen_tunneling_distance(dungeon_t *d){
  int i, j;

  for(i = 1; i < DUNGEON_Y - 1; i++) {
    for(j = 1; j < DUNGEON_X - 1; j++) {
      d->dist_tun_map[i][j] = 0; // Dijkstra from i,j to player_y, player_x
    }
  }
...
int place_character(dungeon_t *d) {
	do {
    player[dim_x] = rand() % DUNGEON_X;
    player[dim_y] = rand() % DUNGEON_Y;
  } while (!in_room(d, player[dim_y], player[dim_x]));
  return 0;
}
...
int gen_dungeon(dungeon_t *d)
{
  ...
  /* Gen Distance Normal */
  gen_distance(d);
  /* Gen distance nont-normal */
  // gen_tunneling_distance(d);
...
void render_dungeon(dungeon_t *d)
{
  ...
  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
    for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
      if(p[dim_y] == player[dim_y] && p[dim_x] == player[dim_x]) {
        putchar('@');
        continue;
      }
      switch (mappair(p)) {
  ...
  putchar('\n');
  /*print first dist map */

  for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
          int dist = distxy(p[dim_x], p[dim_y]);
          if(dist < 10) {
            putchar('0' + dist);
          } else if (dist < 36 ) {
            putchar('a' + (dist - 10));
  ...
  /*print second dist map */

    for (p[dim_y] = 0; p[dim_y] < DUNGEON_Y; p[dim_y]++) {
        for (p[dim_x] = 0; p[dim_x] < DUNGEON_X; p[dim_x]++) {
          int dist = disttunxy(p[dim_x], p[dim_y]);
          if(dist < 10) {
            putchar('0' + dist);
          } else if (dist < 36 ) {
            putchar('a' + (dist - 10));
